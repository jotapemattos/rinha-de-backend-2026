...
for (let i = 0; i < 500; i++) FraudService.scoreTransactionFromBuffer(_warmupBuf, 0, _warmupBuf.length);
  ...
  socket: {
    ...
    data(socket, chunk) {
      ...
      while (offset < buf.length) {
        ...
        const prob = FraudService.scoreTransactionFromBuffer(buf, bodyStart, bodyStart + contentLength);
